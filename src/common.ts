...
import {ProductType, ObjectData, ProductTypeBody} from "./type";
...
const createRandomUser = (Type: ProductType): ProductTypeBody => {
    return {
        id:  faker.datatype.uuid(),
        type: Type,
        name: faker.commerce.productName(),
        amount: Number(faker.random.numeric()),
        price: Type ===  ProductType.GIVE_FOR_FREE ? 0 : Number(faker.commerce.price()),

    };
}

const generateMyRecord = async (Type: ProductType,userName: string) => {
    ...
    const newProduct: ProductTypeBody = {
        id: faker.datatype.uuid(),
        type: Type,
        name: productName,
...
export const giveMeFakeData = (Type: ProductType, amount: number) => {
...
export  const generateMyData = async (userName:string)=> {
    for (let i = 0; i <= LOOP_LENGTH; i++) {
        ...
        if(shopCartData[productType as ProductType ].length) {
...
export const afterJsonFIle = async (shopCartData : ObjectData,Type: ProductType, Name:string) => {
...
export const Avada_Kedavra = async (Name: string) => {
   ...
   const isFinish =  await beforeJsonFile(shopCartData) && await afterJsonFIle(shopCartData,productType,Name)
